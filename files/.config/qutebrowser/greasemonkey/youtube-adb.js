...
(function () {
  ...
  const cssSelectorArr = [
    ...
    `ytd-rich-item-renderer:has(.ytd-display-ad-renderer)`, //首页视频排版广告容器.
    ...
    `ytd-display-ad-renderer`, //显示广告渲染器.
    ...
    `.ytd-promoted-sparkles-web-renderer`, //推广闪烁广告
    `ytd-promoted-sparkles-web-renderer`, //推广闪烁广告渲染器
    `ytd-search-pyv-renderer`, //搜索页推广视频
    `ytd-banner-promo-renderer`, //横幅推广渲染器
  ];
  ...
  function generateRemoveADCssText(cssSelectorArr) {
    ...
    cssSelectorArr.forEach((selector, index) => {
      ...
      enhancedStyles.push(
        `${selector}{display:none!important;visibility:hidden!important;opacity:0!important;height:0!important;width:0!important;margin:0!important;padding:0!important;border:none!important;position:absolute!important;top:-9999px!important;left:-9999px!important;}`,
      );
      ...
      if (selector.includes("ytd-display-ad-renderer")) {
        enhancedStyles.push(
          `ytd-rich-item-renderer:has(.ytd-display-ad-renderer){display:none!important;height:0!important;margin:0!important;padding:0!important;}`,
        );
      }

      if (selector.includes("ytd-ad-slot-renderer")) {
        enhancedStyles.push(
          `ytd-item-section-renderer:has(ytd-ad-slot-renderer){display:none!important;height:0!important;margin:0!important;padding:0!important;}`,
        );
      }
    });
  ...
  function skipAd(mutationsList, observer) {
    ...
    const skipButton =
      document.querySelector(`.html5-video-player .ytp-ad-skip-button`) ||
    ...
    const shortAdMsg =
      document.querySelector(`.html5-video-player .video-ads.ytp-ad-module .ytp-ad-player-overlay`) ||
    ...
    if (skipButton && isAdPlaying) {
      // Additional check: ensure the button is visible and within the video player
      const videoPlayer = document.querySelector(`.html5-video-player`) || document.querySelector(`#movie_player`);
      if (videoPlayer && videoPlayer.contains(skipButton)) {
        const delayTime = 0.5;
    ...
    } else if (shortAdMsg && isAdPlaying) {
      video.currentTime = video.duration; //强制
      log(`强制结束了该广告`);
    }
  }
  ...
  let removePop = (node) => {
    // More specific selector for ad blocker enforcement popups only
    ...
    if (elpopup) {
      ...
      const isAdBlockerWarning =
        popupText.includes("ad blocker") ||
      ...
      if (isAdBlockerWarning) {
        ...
        for (var x = (bdelems || []).length; x--; ) {
          // Only remove backdrops with specific z-index that indicate ad blocker warnings
          if (bdelems[x].style.zIndex === "2201" || bdelems[x].style.zIndex === "2200") {
            bdelems[x].remove();
          }
        }
        resumeVideo();
    ...
    if (
      node.tagName.toLowerCase() === "tp-yt-iron-overlay-backdrop" &&
      (node.style.zIndex === "2201" || node.style.zIndex === "2200")
    ) {
      node.remove();
      resumeVideo();
      console.log("remove ad blocker backdrop", node);
    }
  };
